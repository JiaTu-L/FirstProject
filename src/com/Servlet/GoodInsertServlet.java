...
//            GoodsInfo goodsInfo = this.getGoodsInfoByNormalForm(req);
            ...
            rows = goodsDao.addGoodsInfo(goodsInfo);
        } catch (Exception e) {
            e.printStackTrace();
            errMsg = e.getMessage();
        }
        ...
        if(rows>0){
            out.print("<script type='text/javascript'>alert('添加成功');location.href = 'goodQueryServlet';</script>");
        } else {
            out.print("<script type='text/javascript'>alert('添加失败:" + errMsg + "');history.back();</script>");
        }
    }
